...
面向知识沉淀、社区交流和 AI 知识问答的一体化平台。当前 `docker-linux` 分支基于 4 GB 轻量版，提供 Docker Compose 和 Linux 一键部署脚本；不运行 Nacos、MinIO、RabbitMQ、Elasticsearch 或 Milvus，只用一个小型 Redis 保存会话吊销记录。
...
- 网关使用本地限流；Redis（48 MB 上限）只记录已吊销的会话，退出登录、改密、重置密码或停用账号后旧会话立即失效。Redis 不可用时网关照常服务并记录日志。
- 登录状态保存在 httpOnly Cookie 中，页面脚本读取不到令牌。
...
- 注册、登录、图形验证码、JWT 鉴权和个人资料维护；连续登录失败会临时锁定账号，昵称不能冒充管理员或官方
- 忘记密码：提交申请后由管理员核实身份并签发一次性重置码（30 分钟有效）
- 知识上传、分类、检索、阅读、下载、点赞、收藏和举报，支持 DOCX 正文图片在线预览
...
- 平台名称与公告、注册、AI、知识上传、榜单、社区、评论、私信、通知和反馈开关
- 帖子发布前审核、新用户默认发帖策略、知识文件上限、帖子配图数、评论/私信长度和草稿保留周期配置
- FAQ 维护，以及 AI 提供商、请求地址、模型、温度、知识范围和回复合规规则配置

管理端保存的平台配置会持久化，并由用户、知识、社区、消息和 AI 服务读取执行。数据库密码、JWT 密钥、内部服务令牌和外部 AI 密钥属于部署敏感信息，不会显示在管理端，仍需通过 `.env` 配置。
