        ...
        <template v-else>
          ...
          <section v-else-if="activeView === 'moderation'" class="page-stack">
            ...
            <section class="surface"><el-tabs v-model="moderationTab">
              <el-tab-pane label="知识资源" name="knowledge"><el-table :data="knowledgeFiles"><el-table-column prop="id" label="ID" width="70" /><el-table-column prop="title" label="标题" min-width="220" /><el-table-column prop="fileType" label="格式" width="90" /><el-table-column label="状态" width="110"><template #default="scope">{{ auditLabel(scope.row.auditStatus) }}</template></el-table-column><el-table-column label="操作" width="430"><template #default="scope"><el-button text type="primary" @click="reviewKnowledge(scope.row)">查看内容</el-button><el-button text @click="openKnowledgeMetadata(scope.row)">编辑</el-button><el-button v-if="scope.row.auditStatus === 'PENDING'" text type="success" @click="auditKnowledge(scope.row, 'APPROVED')">通过</el-button><el-button v-else-if="scope.row.auditStatus === 'REJECTED'" text type="success" @click="auditKnowledge(scope.row, 'APPROVED')">重新通过</el-button><el-button v-else-if="scope.row.auditStatus === 'APPROVED'" text type="warning" @click="updateKnowledgeStatus(scope.row, 'HIDDEN')">下架</el-button><el-button v-else-if="scope.row.auditStatus === 'HIDDEN'" text type="success" @click="updateKnowledgeStatus(scope.row, 'APPROVED')">恢复</el-button><el-button text type="danger" @click="deleteAdminKnowledge(scope.row)">删除</el-button></template></el-table-column></el-table></el-tab-pane>
