...
async function loadProfile(){const [user,follows,blocks,history,directory,faqResult]=await Promise.all([getData<UserRecord>(`/user/info?username=${username.value}`),getData<{followedUserIds:number[];followerUserIds:number[]}>(`/user/follows?userId=${currentUserId.value}`),getData<{blockedUserIds:number[]}>(`/user/blocks?userId=${currentUserId.value}`),getData<BehaviorRecord[]>(`/user/behaviors?userId=${currentUserId.value}`),getData<UserRecord[]>('/user/directory'),getData<Faq[]>('/feedback/faqs')]);profileForm.value={userId:user.id,nickname:user.nickname,avatarUrl:user.avatarUrl||'',signature:user.signature||''};displayName.value=user.nickname;followData.value=follows;blockedUserIds.value=blocks.blockedUserIds;behaviors.value=history.slice(-30).reverse();directoryUsers.value=directory.filter(item=>item.id!==currentUserId.value);faqs.value=faqResult;if(!directoryUsers.value.some(item=>item.id===relationForm.value.targetUserId))relationForm.value.targetUserId=directoryUsers.value[0]?.id||0;await Promise.all([loadDrafts(),loadFeedback(),loadMyKnowledge(),loadCollectedPosts()]);}
