          ...
          <section v-else-if="activeView === 'community'" class="page-stack">
            ...
            <div class="feed-layout">
              <section class="feed-column">
                <div class="feed-tabs"><button :class="{ active: feedMode === 'all' }" @click="loadFeed('all')">最新</button><button :class="{ active: feedMode === 'following' }" @click="loadFeed('following')">关注</button><button :class="{ active: feedMode === 'mine' }" @click="loadMyPosts">我的帖子</button></div>
...
async function loadProfile(){const [user,follows,blocks,history,faqResult]=await Promise.all([getData<UserRecord>(`/user/info?username=${username.value}`),getData<{followedUserIds:number[];followerUserIds:number[]}>(`/user/follows?userId=${currentUserId.value}`),getData<{blockedUserIds:number[]}>(`/user/blocks?userId=${currentUserId.value}`),getData<BehaviorRecord[]>(`/user/behaviors?userId=${currentUserId.value}`),getData<Faq[]>('/feedback/faqs')]);profileForm.value={userId:user.id,nickname:user.nickname,avatarUrl:user.avatarUrl||'',signature:user.signature||''};displayName.value=user.nickname;followData.value=follows;blockedUserIds.value=blocks.blockedUserIds;behaviors.value=history.slice(-30).reverse();faqs.value=faqResult;await Promise.all([loadDrafts(),loadFeedback(),loadMyKnowledge(),loadCollectedPosts()]);}
