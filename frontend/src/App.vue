...
async function completeAuthentication(result:AuthResult){
  ...
  username.value=result.user.username;displayName.value=result.user.nickname;avatarUrl.value=result.user.avatarUrl||'';role.value=result.role;currentUserId.value=result.user.id;
...
async function refreshCurrentView(){ viewLoading.value=true; viewError.value=''; if(portal.value==='client')void refreshUnreadCount(); try { if(portal.value==='admin'){ if(activeView.value==='dashboard') await loadAdminDashboard(); else if(activeView.value==='moderation') await loadModeration(); else if(activeView.value==='analytics') await loadAnalytics(); else if(activeView.value==='users') await loadAdminUsers(true); else if(activeView.value==='tickets') await loadTicketsAdmin(); else await loadSystemAdmin(); } else { if(['home','knowledge'].includes(activeView.value)) await loadKnowledge(); if(activeView.value==='home')await Promise.all([loadFeed('following'),loadCommunityPostCount()]);if(activeView.value==='forum')await loadFeed(feedMode.value==='author'?'author':feedMode.value==='mine'?'mine':'all');if(activeView.value==='square')await loadFeed(feedMode.value==='author'?'author':feedMode.value==='mine'?'mine':'following'); if(['home','messages'].includes(activeView.value)) await loadMessageData(); if(activeView.value==='ai') await loadAiHistory(); if(activeView.value==='profile') await loadProfile(); if(activeView.value==='home') await loadFeedback(); } } catch(error){ viewError.value=toUserMessage(error,'页面加载失败，请重试'); notifyError(error); } finally { viewLoading.value=false; } }
...
async function saveProfile(){const nickname=profileForm.value.nickname.trim();if(role.value!=='ADMIN'&&nickname!==displayName.value&&nicknameImpersonatesStaff(nickname)){ElMessage.warning('昵称不能冒充平台管理员、官方或客服，请更换');return;}const user=await postData<UserRecord>('/user/profile',profileForm.value);displayName.value=user.nickname;avatarUrl.value=user.avatarUrl||'';setStoredValue('ai-knowledge-name',user.nickname);setStoredValue('ai-knowledge-avatar',avatarUrl.value);ElMessage.success('资料已保存');}
...
async function saveUserGovernance(){await postData('/user/admin/governance',userGovernanceForm.value);userGovernanceDialog.value=false;await loadAdminUsers(true);ElMessage.success('用户资料与权限已更新');}
...
async function loadAdminUsers(reset=true){
  ...
  try{
    ...
    if(overview)adminOverview.value={...adminOverview.value,userAdmin:overview};
  } finally { if(token===adminUserToken)adminUserLoading.value=false; }
}
type ResetRequestRecord = { id:number; userId:number; username:string; nickname:string; accountStatus:string; contact:string; status:string; codeExpired:boolean; failedAttempts:number; note:string; createdAt:string; updatedAt:string };
...
watch(resetRequestStatus,()=>{if(portal.value==='admin'&&activeView.value==='users')void loadResetRequests();});
...
watch([adminUserKeyword,adminUserStatusFilter,adminUserRoleFilter],()=>{
  if(portal.value!=='admin'||activeView.value!=='users')return;
