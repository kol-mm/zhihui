          ...
          <section v-else-if="activeView === 'community'" class="page-stack">
            ...
            <div class="feed-layout">
              <section class="feed-column">
                <div class="feed-tabs"><button :class="{ active: feedMode === 'following' }" @click="loadFeed('following')">关注动态</button><button :class="{ active: feedMode === 'mine' }" @click="loadMyPosts">我的帖子</button><button v-if="feedMode === 'author'" class="active" @click="loadFeed('following')">{{ communityUser(authorFilterUserId).nickname }} 的动态</button></div>
...
type KnowledgeFile = { id:number; userId:number; categoryId?:number; title:string; fileType:string; auditStatus:string; fileUrl?:string; content?:string; imageUrls?:string[]; coverUrl?:string; views?:number; downloads?:number; likes?:number; liked?:boolean; collected?:boolean };
...
type Draft = { id:number; title:string; content:string; userId:number; updatedAt?:string };
...
const feedMode = ref<'all'|'following'|'mine'|'author'>('following'); const authorFilterUserId = ref(0); const moderationTab = ref('knowledge'); const governanceTab = ref('comments');
...
async function uploadKnowledge(){
  ...
  try{
    ...
    if(selectedKnowledgeFile.value){
      ...
      if(knowledgeForm.value.categoryId)form.append('categoryId',String(knowledgeForm.value.categoryId));
    ...
    knowledgeDialog.value=false;selectedKnowledgeFile.value=undefined;knowledgeForm.value.title='';knowledgeForm.value.content='';knowledgeForm.value.categoryId=0;
...
async function loadAuthorPostsFromProfile(userId:number){activeView.value='community';await loadAuthorPosts(userId);}
