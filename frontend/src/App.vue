  ...
  <div v-if="!authenticated" class="auth-screen">
    ...
    <el-dialog v-model="registerDialog" title="注册社区账号" width="min(460px, 92vw)">
      <el-form label-position="top">
        ...
        <el-form-item label="人机验证"><div class="captcha-control"><div class="captcha-challenge"><img v-if="captchaImage" :src="captchaImage" alt="计算验证码" /><span v-else>{{ captchaLoading ? '正在获取验证码…' : '验证码暂不可用' }}</span></div><div class="captcha-row"><el-input v-model="registerForm.captchaAnswer" inputmode="numeric" maxlength="3" placeholder="请输入图片中的计算结果" /><el-button type="info" plain :loading="captchaLoading" :disabled="captchaCooldownRemaining > 0" @click="loadCaptcha">{{ captchaCooldownRemaining > 0 ? `${captchaCooldownRemaining} 秒后可换` : '换一张' }}</el-button></div><p class="captcha-hint">请计算图片中的算式，用于确认当前操作由真人完成。</p></div></el-form-item>
      </el-form>
  ...
  <div v-else class="app-layout" :class="{ 'sidebar-collapsed': sidebarCollapsed }">
    <aside class="sidebar" :class="{ open: mobileMenuOpen }">
      <div class="sidebar-brand">
        ...
        <div><strong>知汇</strong><span>人工智能知识社区</span></div>
      </div>
    ...
    <el-dialog v-model="postDialog" :title="postDialogTitle" width="min(560px, 92vw)" destroy-on-close @closed="resetPostEditor"><el-form label-position="top"><el-form-item label="标题"><el-input v-model="postForm.title" /></el-form-item><el-form-item label="正文"><el-input v-model="postForm.content" type="textarea" :rows="6" /></el-form-item><el-form-item label="帖子配图"><el-upload v-model:file-list="postImageFiles" list-type="picture-card" :auto-upload="false" :limit="9" accept="image/jpeg,image/png,image/gif,image/webp" :on-change="handlePostImages" :on-remove="handlePostImages"><Plus /></el-upload><div class="el-upload__tip">最多 9 张，支持 JPEG、PNG、GIF、WebP，单张不超过 10 MB</div></el-form-item></el-form><template #footer><el-button @click="postDialog = false">取消</el-button><el-button v-if="!editingPostId" :loading="busy" @click="saveDraft">{{ editingDraftId ? '保存草稿' : '存为草稿' }}</el-button><el-button type="primary" :loading="busy" @click="createPost">{{ editingPostId ? '保存修改' : editingDraftId ? '发布草稿' : '发布' }}</el-button></template></el-dialog>
    ...
    <el-dialog v-model="conversationDialog" title="发起私信" width="min(460px, 92vw)"><el-form label-position="top"><el-form-item label="对方完整用户名"><div class="exact-user-search"><el-input v-model="conversationUsername" placeholder="输入完整用户名" clearable @clear="conversationTargetUser = undefined; conversationTargetId = 0" @keyup.enter="resolveConversationUser" /><el-button :icon="Search" @click="resolveConversationUser">查找</el-button></div></el-form-item></el-form><div v-if="conversationTargetUser" class="exact-user-result"><strong>{{ conversationTargetUser.nickname }}</strong><span>@{{ conversationTargetUser.username }}</span></div><template #footer><el-button @click="conversationDialog = false">取消</el-button><el-button type="primary" :disabled="!conversationTargetId" @click="createConversation">开始聊天</el-button></template></el-dialog>
...
import { ArrowRight, Bell, ChatDotRound, ChatLineRound, CollectionTag, DataAnalysis, Delete, Document, Download, Edit, EditPen, Files, House, Loading, MagicStick, Menu, Message, MoreFilled, Plus, Promotion, Reading, Refresh, Search, Setting, Star, SwitchButton, Tickets, Upload, UploadFilled, User, UserFilled, View, Warning } from '@element-plus/icons-vue';
...
const aiConfig = ref({ data_source_scope:'all-approved', match_limit:5, compliance_rule:'answer-with-references', provider:'local', model:'local-rag', base_url:'', request_url:'', temperature:0.2, max_upload_mb:25, notifications_enabled:true, community_enabled:true, selected_file_ids:[] as number[] });
...
const knowledgeFiles = ref<KnowledgeFile[]>([]); const feedPosts = ref<Post[]>([]); const collectedPosts = ref<Post[]>([]); const tickets = ref<Ticket[]>([]); const adminTickets = ref<Ticket[]>([]);
...
function selectView(key:string){ if(key==='governance'){governanceDialog.value=true;mobileMenuOpen.value=false;loadGovernance();return;} if(key==='notifications'){mobileMenuOpen.value=false;openNotifications();return;} returnToRoot();if(key==='forum')feedMode.value='all';if(key==='square')feedMode.value='following';activeView.value=key; mobileMenuOpen.value=false; refreshCurrentView(); }
async function refreshCurrentView(){ viewLoading.value=true; viewError.value=''; try { if(portal.value==='admin'){ if(activeView.value==='dashboard') await loadAdminDashboard(); else if(activeView.value==='moderation') await loadModeration(); else if(activeView.value==='analytics') await loadAnalytics(); else if(activeView.value==='users') adminUsers.value=await getData('/user/admin/users'); else if(activeView.value==='tickets') await loadTicketsAdmin(); else await loadSystemAdmin(); } else { if(['home','knowledge'].includes(activeView.value)) await loadKnowledge(); if(activeView.value==='home')await loadFeed('following');if(activeView.value==='forum')await loadFeed(feedMode.value==='author'?'author':feedMode.value==='mine'?'mine':'all');if(activeView.value==='square')await loadFeed(feedMode.value==='author'?'author':feedMode.value==='mine'?'mine':'following'); if(['home','messages'].includes(activeView.value)) await loadMessageData(); if(activeView.value==='ai') await loadAiHistory(); if(activeView.value==='profile') await loadProfile(); if(activeView.value==='home') await loadFeedback(); } } catch(error){ viewError.value=toUserMessage(error,'页面加载失败，请重试'); notifyError(error); } finally { viewLoading.value=false; } }
...
async function quickComment(post:Post){const {value}=await ElMessageBox.prompt('输入公开评论内容','快捷评论',{inputPattern:/\S+/,inputErrorMessage:'评论不能为空',confirmButtonText:'发布'});await postData('/square/quick-comment',{postId:post.id,content:value});await recordBehavior('COMMENT','POST',post.id);ElMessage.success('评论已发布');}
...
function newAiSession(){aiSessionId.value=undefined;aiMessages.value=[];aiQuestion.value='';}
...
async function removeAvatar(){await ElMessageBox.confirm('确认删除当前头像？','删除头像',{type:'warning'});const user=await postData<UserRecord>('/user/profile',{...profileForm.value,avatarUrl:''});profileForm.value.avatarUrl='';avatarUrl.value=user.avatarUrl||'';localStorage.setItem('ai-knowledge-avatar',avatarUrl.value);ElMessage.success('头像已删除');}
...
async function cleanupExpiredDrafts(){await ElMessageBox.confirm('确认清理 30 天前未更新的草稿？','清理过期草稿',{type:'warning'});const result=await deleteData<{removed:number}>('/post/admin/drafts/expired',{retentionDays:30});await loadGovernance();ElMessage.success(`已清理 ${result.removed} 份过期草稿`);}
...
async function loadSystemAdmin(){const [overview,events,files]=await Promise.all([getData<Record<string,unknown>>('/ai/admin/overview'),getData<EventRecord[]>('/event/list?limit=50'),getData<KnowledgeFile[]>('/knowledge/list?includeAll=true')]);aiOverview.value=overview;adminEvents.value=events;aiSourceFiles.value=files.filter(file=>file.auditStatus==='APPROVED');const config=overview.configuration as typeof aiConfig.value|undefined;if(config)aiConfig.value={...aiConfig.value,...config,selected_file_ids:Array.isArray(config.selected_file_ids)?config.selected_file_ids:[]};}
async function saveAiConfig(){if(aiConfig.value.data_source_scope==='admin-selected'&&!aiConfig.value.selected_file_ids.length){ElMessage.warning('请至少选择一个 AI 知识来源');return;}await postData('/ai/admin/config',aiConfig.value);ElMessage.success('AI 配置已保存并立即生效');await loadSystemAdmin();}
