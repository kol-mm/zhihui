            ...
            <div class="feed-layout">
              <section class="feed-column">
                ...
                <article v-for="post in feedPosts" :key="post.id" class="post-card">
                  <div class="post-author"><button class="post-author-link" @click="loadAuthorPosts(post.userId)"><div class="user-avatar"><img v-if="communityUser(post.userId).avatarUrl" :src="resolveApiUrl(communityUser(post.userId).avatarUrl || '')" alt="头像" /><span v-else>{{ communityUser(post.userId).nickname.slice(0, 1).toUpperCase() }}</span></div><div><strong>{{ communityUser(post.userId).nickname }}</strong><span>@{{ communityUser(post.userId).username }} · 帖子 #{{ post.id }}</span></div></button><el-tag v-if="post.status !== 'PUBLISHED'" type="warning">{{ postStatusLabel(post.status) }}</el-tag></div>
        ...
        <template v-else>
          ...
          <section v-else-if="activeView === 'moderation'" class="page-stack">
            ...
            <section class="surface admin-list-surface">
              ...
              <el-tab-pane :label="`待审帖子 ${pendingAuditPosts.length}`" name="posts"><el-table class="admin-desktop-table" :data="filteredPendingAuditPosts"><el-table-column prop="id" label="ID" width="70" /><el-table-column prop="title" label="标题" min-width="220" /><el-table-column prop="userId" label="用户" width="90" /><el-table-column label="状态" width="110"><template #default="scope">{{ postStatusLabel(scope.row.status) }}</template></el-table-column><el-table-column label="操作" width="250"><template #default="scope"><el-button text type="primary" @click="reviewPost(scope.row)">查看内容</el-button><el-button text type="success" :loading="auditingPostId === scope.row.id" :disabled="auditingPostId !== 0" @click="auditPost(scope.row, 'PUBLISHED')">发布</el-button><el-button text type="danger" :loading="auditingPostId === scope.row.id" :disabled="auditingPostId !== 0" @click="auditPost(scope.row, 'HIDDEN')">驳回</el-button></template></el-table-column></el-table><div class="admin-mobile-cards"><article v-for="post in filteredPendingAuditPosts" :key="post.id"><header><strong>帖子 #{{ post.id }}</strong><el-tag size="small" type="warning">待审核</el-tag></header><h4>{{ post.title }}</h4><p>作者 #{{ post.userId }}</p><footer><el-button @click="reviewPost(post)">查看</el-button><el-button type="success" :loading="auditingPostId===post.id" @click="auditPost(post,'PUBLISHED')">发布</el-button><el-button type="danger" plain :loading="auditingPostId===post.id" @click="auditPost(post,'HIDDEN')">驳回</el-button></footer></article><el-empty v-if="!filteredPendingAuditPosts.length" description="当前没有待审帖子" /></div></el-tab-pane>
              <el-tab-pane :label="`帖子管理 ${managedPosts.length}`" name="post-management"><el-table class="admin-desktop-table" :data="filteredManagedPosts"><el-table-column prop="id" label="ID" width="70" /><el-table-column prop="title" label="标题" min-width="220" /><el-table-column prop="userId" label="用户" width="90" /><el-table-column label="状态" width="110"><template #default="scope">{{ postStatusLabel(scope.row.status) }}</template></el-table-column><el-table-column label="操作" width="220"><template #default="scope"><el-button text type="primary" @click="reviewPost(scope.row)">查看内容</el-button><el-button v-if="scope.row.status === 'PUBLISHED'" text type="warning" :loading="auditingPostId === scope.row.id" @click="auditPost(scope.row, 'HIDDEN')">隐藏</el-button><el-button v-else text type="success" :loading="auditingPostId === scope.row.id" @click="auditPost(scope.row, 'PUBLISHED')">恢复</el-button></template></el-table-column></el-table><div class="admin-mobile-cards"><article v-for="post in filteredManagedPosts" :key="post.id"><header><strong>帖子 #{{ post.id }}</strong><el-tag size="small" :type="post.status==='PUBLISHED'?'success':'info'">{{ postStatusLabel(post.status) }}</el-tag></header><h4>{{ post.title }}</h4><p>作者 #{{ post.userId }}</p><footer><el-button @click="reviewPost(post)">查看</el-button><el-button :type="post.status==='PUBLISHED'?'warning':'success'" plain :loading="auditingPostId===post.id" @click="auditPost(post,post.status==='PUBLISHED'?'HIDDEN':'PUBLISHED')">{{ post.status==='PUBLISHED'?'隐藏':'恢复' }}</el-button></footer></article><el-empty v-if="!filteredManagedPosts.length" description="没有匹配的帖子" /></div></el-tab-pane>
            </el-tabs></section>
          </section>

          <section v-else-if="activeView === 'users'" class="page-stack"><div class="page-toolbar"><div><h3>用户管理</h3><p>账号资料、状态、权限和个人内容治理</p></div><el-button :icon="Download" @click="exportAdminUsers">导出筛选结果</el-button></div><section class="surface admin-list-surface"><div class="admin-filter-bar admin-user-filters"><el-input v-model="adminUserKeyword" :prefix-icon="Search" clearable placeholder="搜索 ID、用户名或昵称" /><el-select v-model="adminUserRoleFilter" clearable placeholder="全部角色"><el-option label="管理员" value="ADMIN" /><el-option label="普通用户" value="USER" /></el-select><el-select v-model="adminUserStatusFilter" clearable placeholder="全部状态"><el-option label="正常" value="ACTIVE" /><el-option label="已停用" value="DISABLED" /></el-select><span>找到 <strong>{{ filteredAdminUsers.length }}</strong> 位</span></div><div class="admin-summary-strip"><span>全部<strong>{{ adminUsers.length }}</strong></span><span>正常<strong>{{ adminUsers.filter(item=>item.status==='ACTIVE').length }}</strong></span><span>已停用<strong>{{ adminUsers.filter(item=>item.status!=='ACTIVE').length }}</strong></span><span>管理员<strong>{{ adminUsers.filter(item=>item.role==='ADMIN').length }}</strong></span></div><el-table class="admin-desktop-table" :data="filteredAdminUsers"><el-table-column label="用户" min-width="230"><template #default="scope"><div class="table-user"><div class="mini-avatar">{{ scope.row.nickname.slice(0, 1) }}</div><span><strong>{{ scope.row.nickname }}</strong><small>@{{ scope.row.username }}</small></span></div></template></el-table-column><el-table-column label="角色" width="110"><template #default="scope">{{ roleLabel(scope.row.role) }}</template></el-table-column><el-table-column label="发帖权限" width="130"><template #default="scope">{{ publishPolicyLabel(scope.row.publishPolicy) }}</template></el-table-column><el-table-column prop="status" label="状态" width="110"><template #default="scope"><el-tag :type="scope.row.status === 'ACTIVE' ? 'success' : 'danger'">{{ scope.row.status === 'ACTIVE' ? '正常' : '已停用' }}</el-tag></template></el-table-column><el-table-column label="操作" width="230"><template #default="scope"><el-button text type="primary" @click="openUserGovernance(scope.row)">资料与治理</el-button><el-button text :type="scope.row.status === 'ACTIVE' ? 'danger' : 'success'" @click="toggleUserStatus(scope.row)">{{ scope.row.status === 'ACTIVE' ? '禁用' : '恢复' }}</el-button></template></el-table-column></el-table><div class="admin-mobile-cards admin-user-cards"><article v-for="user in filteredAdminUsers" :key="user.id"><header><div class="table-user"><div class="mini-avatar">{{ user.nickname.slice(0,1) }}</div><span><strong>{{ user.nickname }}</strong><small>@{{ user.username }} · #{{ user.id }}</small></span></div><el-tag size="small" :type="user.status==='ACTIVE'?'success':'danger'">{{ user.status==='ACTIVE'?'正常':'已停用' }}</el-tag></header><p>{{ roleLabel(user.role) }} · {{ publishPolicyLabel(user.publishPolicy) }}</p><footer><el-button type="primary" plain @click="openUserGovernance(user)">资料与治理</el-button><el-button :type="user.status==='ACTIVE'?'danger':'success'" plain @click="toggleUserStatus(user)">{{ user.status==='ACTIVE'?'禁用账号':'恢复账号' }}</el-button></footer></article><el-empty v-if="!filteredAdminUsers.length" description="没有匹配的用户" /></div></section></section>

          <section v-else-if="activeView === 'analytics'" class="page-stack"><div class="page-toolbar"><div><h3>平台数据统计</h3><p>社区产出、互动热度、违规风险和反馈趋势</p></div><el-segmented v-model="analyticsDays" :options="[{label:'近 7 天',value:7},{label:'近 30 天',value:30},{label:'近 90 天',value:90}]" /></div><div class="metrics-grid admin-metrics"><div class="metric-tile"><span class="metric-icon green"><UserFilled /></span><div><strong>{{ metricValue('userAdmin','activeUsers') }}</strong><span>活跃账号</span><small>全站正常用户</small></div></div><div class="metric-tile"><span class="metric-icon blue"><Files /></span><div><strong>{{ analyticsKnowledge.length }}</strong><span>知识产出</span><small>所选周期内</small></div></div><div class="metric-tile"><span class="metric-icon amber"><ChatDotRound /></span><div><strong>{{ analyticsPosts.length }}</strong><span>社区帖子</span><small>已发布内容</small></div></div><div class="metric-tile"><span class="metric-icon red"><DataAnalysis /></span><div><strong>{{ analyticsInteractions }}</strong><span>内容互动</span><small>浏览、下载与点赞</small></div></div></div><section class="surface"><div class="surface-head"><div><h3>近 {{ Math.min(analyticsDays,14) }} 天内容趋势</h3><p>每日新增知识、帖子与反馈工单</p></div><div class="analytics-legend"><span class="knowledge">知识</span><span class="post">帖子</span><span class="ticket">工单</span></div></div><div class="analytics-trend"><div v-for="item in analyticsTrend" :key="item.label" class="trend-day"><div class="trend-bars"><i class="knowledge" :style="{height:`${Math.max(3,item.knowledge/analyticsTrendMax*100)}%`}" :title="`知识 ${item.knowledge}`"></i><i class="post" :style="{height:`${Math.max(3,item.posts/analyticsTrendMax*100)}%`}" :title="`帖子 ${item.posts}`"></i><i class="ticket" :style="{height:`${Math.max(3,item.tickets/analyticsTrendMax*100)}%`}" :title="`工单 ${item.tickets}`"></i></div><small>{{ item.label }}</small></div></div></section><div class="two-column"><section class="surface"><div class="surface-head"><div><h3>热门知识</h3><p>按浏览、下载和点赞综合排序</p></div></div><div class="analytics-ranking"><article v-for="(file,index) in topKnowledge" :key="file.id"><b>{{ index+1 }}</b><span><strong>{{ file.title }}</strong><small>浏览 {{ file.views||0 }} · 下载 {{ file.downloads||0 }} · 点赞 {{ file.likes||0 }}</small></span></article></div><el-empty v-if="!topKnowledge.length" description="暂无知识数据" /></section><section class="surface"><div class="surface-head"><div><h3>热门帖子与风险</h3><p>互动内容及待处置事项</p></div><el-tag type="danger" effect="plain">{{ knowledgeReports.length+userReports.length }} 项举报</el-tag></div><div class="analytics-ranking"><article v-for="(post,index) in topPosts" :key="post.id"><b>{{ index+1 }}</b><span><strong>{{ post.title }}</strong><small>点赞 {{ post.likes||0 }} · 作者 #{{ post.userId }}</small></span></article></div><el-empty v-if="!topPosts.length" description="暂无社区数据" /></section></div><section class="surface"><div class="surface-head"><div><h3>反馈结构</h3><p>所选周期内工单类型与处理进度</p></div></div><div class="governance-user-summary"><span>系统问题 <strong>{{ analyticsTickets.filter(item=>item.type==='BUG').length }}</strong></span><span>产品建议 <strong>{{ analyticsTickets.filter(item=>item.type==='SUGGESTION').length }}</strong></span><span>客服咨询 <strong>{{ analyticsTickets.filter(item=>item.type==='SUPPORT').length }}</strong></span><span>已完结 <strong>{{ analyticsTickets.filter(item=>item.status==='RESOLVED').length }}</strong></span></div></section></section>
          ...
          <section v-else-if="activeView === 'tickets'" class="page-stack">
            ...
            <div class="two-column">
              <section class="surface"><div class="surface-head"><div><h3>反馈工单</h3><p>{{ filteredAdminTickets.length }} / {{ adminTickets.length }} 条记录</p></div></div><div class="admin-filter-bar ticket-filter-bar"><el-input v-model="adminTicketKeyword" :prefix-icon="Search" clearable placeholder="搜索编号、用户或内容" /><el-select v-model="adminTicketStatus" clearable placeholder="全部状态"><el-option label="待处理" value="PENDING" /><el-option label="处理中" value="PROCESSING" /><el-option label="已解决" value="RESOLVED" /></el-select></div><article v-for="ticket in filteredAdminTickets" :key="ticket.id" class="ticket-row"><div><strong>#{{ ticket.id }} · {{ ticketTypeLabel(ticket.type) }}</strong><p>{{ ticket.content }}</p><small>用户 #{{ ticket.userId }}<template v-if="ticket.reply"> · 当前回复：{{ ticket.reply }}</template></small><small v-if="ticket.assignedAt">分配时间：{{ formatDate(ticket.assignedAt) }}</small></div><div class="ticket-actions"><el-select :model-value="ticket.assigneeUserId||0" size="small" class="ticket-assignee" @change="assignTicket(ticket, $event)"><el-option label="未分配" :value="0" /><el-option v-for="user in adminUsers.filter(item=>item.role==='ADMIN'&&item.status==='ACTIVE')" :key="user.id" :label="user.nickname" :value="user.id" /></el-select><el-tag :type="ticket.status === 'RESOLVED' ? 'success' : 'warning'">{{ ticketStatusLabel(ticket.status) }}</el-tag><el-button text type="primary" @click="openTicketReply(ticket)">处理</el-button></div></article><el-empty v-if="!filteredAdminTickets.length" description="没有匹配的工单" /></section>
...
import { ArrowLeft, ArrowRight, Bell, ChatDotRound, ChatLineRound, Close, CollectionTag, DataAnalysis, Delete, Document, Download, Edit, EditPen, Files, House, Loading, MagicStick, Menu, Message, MoreFilled, Plus, Promotion, Reading, Refresh, Search, Setting, Star, SwitchButton, Tickets, Upload, UploadFilled, User, UserFilled, View, Warning } from '@element-plus/icons-vue';
...
const aiConfig = ref({ platform_name:'知汇', platform_notice:'', registration_enabled:true, ai_chat_enabled:true, knowledge_upload_enabled:true, user_ranking_enabled:true, community_enabled:true, comments_enabled:true, private_messages_enabled:true, notifications_enabled:true, feedback_enabled:true, post_audit_required:true, default_publish_policy:'STANDARD', max_upload_mb:25, max_post_images:9, max_comment_length:2000, max_message_length:2000, draft_retention_days:30, data_source_scope:'all-approved', match_limit:5, compliance_rule:'answer-with-references', provider:'local', model:'local-rag', base_url:'', request_url:'', temperature:0.2, selected_file_ids:[] as number[] });
...
const analyticsCutoff = computed(() => Date.now() - analyticsDays.value * 86400000);
const approvedKnowledge = computed(() => knowledgeFiles.value.filter(item => item.auditStatus === 'APPROVED'));
const pendingAuditPosts = computed(() => feedPosts.value.filter(item => item.status === 'PENDING'));
const managedPosts = computed(() => feedPosts.value.filter(item => item.status === 'PUBLISHED' || item.status === 'HIDDEN'));
const filteredModerationKnowledge = computed(() => knowledgeFiles.value.filter(file=>adminMatches(adminModerationKeyword.value,file.id,file.title,file.fileType)&&(!adminModerationStatus.value||file.auditStatus===adminModerationStatus.value)));
...
const filteredPendingAuditPosts = computed(() => pendingAuditPosts.value.filter(post=>adminMatches(adminModerationKeyword.value,post.id,post.userId,post.title)&&(!adminModerationStatus.value||post.status===adminModerationStatus.value)));
const filteredManagedPosts = computed(() => managedPosts.value.filter(post=>adminMatches(adminModerationKeyword.value,post.id,post.userId,post.title)&&(!adminModerationStatus.value||post.status===adminModerationStatus.value)));
...
const moderationResultCount = computed(()=>({knowledge:filteredModerationKnowledge.value.length,reports:filteredKnowledgeReports.value.length,users:filteredUserReports.value.length,posts:filteredPendingAuditPosts.value.length,'post-management':filteredManagedPosts.value.length}[moderationTab.value]||0));
const filteredAdminTickets = computed(()=>adminTickets.value.filter(ticket=>adminMatches(adminTicketKeyword.value,ticket.id,ticket.userId,ticket.type,ticket.content,ticket.reply)&&(!adminTicketStatus.value||ticket.status===adminTicketStatus.value)));
const moderationOpenCount = computed(()=>knowledgeFiles.value.filter(file=>file.auditStatus==='PENDING').length+pendingAuditPosts.value.length+knowledgeReports.value.filter(report=>report.status!=='RESOLVED').length+userReports.value.filter(report=>report.status!=='RESOLVED').length);
const ticketOpenCount = computed(()=>adminTickets.value.filter(ticket=>ticket.status!=='RESOLVED').length);
...
const publishedPosts = computed(() => feedPosts.value.filter(item => item.status === 'PUBLISHED'));
const analyticsKnowledge = computed(() => approvedKnowledge.value.filter(item => !item.createdAt || new Date(item.createdAt).getTime() >= analyticsCutoff.value));
const analyticsPosts = computed(() => publishedPosts.value.filter(item => !item.createdAt || new Date(item.createdAt).getTime() >= analyticsCutoff.value));
const analyticsTickets = computed(() => adminTickets.value.filter(item => !item.createdAt || new Date(item.createdAt).getTime() >= analyticsCutoff.value));
const analyticsInteractions = computed(() => analyticsKnowledge.value.reduce((sum,item)=>sum+(item.views||0)+(item.downloads||0)+(item.likes||0),0)+analyticsPosts.value.reduce((sum,item)=>sum+(item.likes||0),0));
const topKnowledge = computed(() => [...approvedKnowledge.value].sort((a,b)=>((b.views||0)+(b.downloads||0)*2+(b.likes||0)*2)-((a.views||0)+(a.downloads||0)*2+(a.likes||0)*2)).slice(0,5));
const topPosts = computed(() => [...publishedPosts.value].sort((a,b)=>(b.likes||0)-(a.likes||0)).slice(0,5));
const analyticsTrend = computed(() => Array.from({length:Math.min(analyticsDays.value,14)},(_,index)=>{const day=new Date();day.setHours(0,0,0,0);day.setDate(day.getDate()-(Math.min(analyticsDays.value,14)-1-index));const next=day.getTime()+86400000;const inDay=(value?:string)=>{const time=value?new Date(value).getTime():0;return time>=day.getTime()&&time<next;};return{label:`${day.getMonth()+1}/${day.getDate()}`,knowledge:approvedKnowledge.value.filter(item=>inDay(item.createdAt)).length,posts:publishedPosts.value.filter(item=>inDay(item.createdAt)).length,tickets:adminTickets.value.filter(item=>inDay(item.createdAt)).length};}));
...
async function loadCaptcha(options?:{silent?:boolean}){
  ...
  try{
    ...
    if(result.captchaId&&result.image){captchaImage.value=result.image;loginForm.value.captchaId=result.captchaId;registerForm.value.captchaId=result.captchaId;const expiresAt=result.expiresInSeconds?Date.now()+result.expiresInSeconds*1000:result.expiresAt;if(expiresAt)scheduleCaptchaExpiry(expiresAt);}
    ...
    loginForm.value.captchaAnswer='';registerForm.value.captchaAnswer='';startCaptchaCooldown(result.refreshAfterSeconds || 10);
...
async function completeAuthentication(result:AuthResult){
  ...
  setStoredValue('ai-knowledge-username',username.value);setStoredValue('ai-knowledge-name',displayName.value);setStoredValue('ai-knowledge-avatar',avatarUrl.value);setStoredValue('ai-knowledge-role',role.value);setStoredValue('ai-knowledge-user-id',String(currentUserId.value));
...
async function likeKnowledge(file:KnowledgeFile){const result=await postData<KnowledgeLikeResult>('/knowledge/like',{fileId:file.id});file.likes=result.likes;file.liked=result.liked;if(result.liked){await recordBehavior('LIKE','KNOWLEDGE',file.id);ElMessage.success('已点赞');}else{if(knowledgeActivityType.value==='LIKED')myKnowledge.value=myKnowledge.value.filter(item=>item.id!==file.id);ElMessage.success('已取消点赞');}}
...
async function loadProfile(){const [user,follows,blocks,history,faqResult]=await Promise.all([getData<UserRecord>(`/user/info?username=${username.value}`),getData<{followedUserIds:number[];followerUserIds:number[]}>(`/user/follows?userId=${currentUserId.value}`),getData<{blockedUserIds:number[]}>(`/user/blocks?userId=${currentUserId.value}`),getData<BehaviorRecord[]>(`/user/behaviors?userId=${currentUserId.value}`),getData<Faq[]>('/feedback/faqs')]);profileForm.value={userId:user.id,nickname:user.nickname,avatarUrl:user.avatarUrl||'',signature:user.signature||''};displayName.value=user.nickname;followData.value=follows;blockedUserIds.value=blocks.blockedUserIds;behaviors.value=history.slice(-100).reverse();faqs.value=faqResult;await loadUserSummaries([...(follows.followedUserIds||[]),...(follows.followerUserIds||[])]);await Promise.all([loadDrafts(),loadFeedback(),loadMyKnowledge(),loadCollectedPosts()]);}
async function saveProfile(){const user=await postData<UserRecord>('/user/profile',profileForm.value);displayName.value=user.nickname;avatarUrl.value=user.avatarUrl||'';setStoredValue('ai-knowledge-name',user.nickname);setStoredValue('ai-knowledge-avatar',avatarUrl.value);ElMessage.success('资料已保存');}
async function removeAvatar(){await ElMessageBox.confirm('确认删除当前头像？','删除头像',{type:'warning'});const user=await postData<UserRecord>('/user/profile',{...profileForm.value,avatarUrl:''});profileForm.value.avatarUrl='';avatarUrl.value=user.avatarUrl||'';setStoredValue('ai-knowledge-avatar',avatarUrl.value);ElMessage.success('头像已删除');}
...
async function updateKnowledgeStatus(file:KnowledgeFile,auditStatus:string){const updated=await putData<KnowledgeFile>('/knowledge/admin/file',{fileId:file.id,auditStatus});await syncKnowledgeIndex(updated);await loadModeration();ElMessage.success(auditStatus==='APPROVED'?'资源已恢复':'资源已下架');}
...
async function auditKnowledge(file:KnowledgeFile,status:string){await postData('/knowledge/admin/audit',{fileId:file.id,auditStatus:status,reason:'管理员审核'});if(status==='APPROVED'){const detail=await getData<KnowledgeFile>(`/knowledge/admin/preview?fileId=${file.id}`);if(detail.content?.trim())await postData('/ai/parse',{file_id:file.id,title:detail.title,text:detail.content});}else{await deleteData(`/ai/admin/index/file/${file.id}`);}if(reviewingKnowledge.value&&selectedKnowledge.value?.id===file.id){readerDialog.value=false;reviewingKnowledge.value=false;}await loadModeration();ElMessage.success('审核状态与 AI 索引已更新');}
async function auditPost(post:Post,status:string){if(auditingPostId.value)return;auditingPostId.value=post.id;try{const result=await postData<{post:Post}>('/post/admin/audit',{postId:post.id,status,reason:'管理员审核'});feedPosts.value=feedPosts.value.map(item=>item.id===post.id?result.post:item);if(selectedReviewPost.value?.id===post.id){selectedReviewPost.value=result.post;postReviewDialog.value=false;}await loadModeration();ElMessage.success(status==='PUBLISHED'?(post.status==='HIDDEN'?'帖子已恢复':'帖子已发布'):(post.status==='PENDING'?'帖子已驳回并隐藏':'帖子已隐藏'));}catch(error){notifyError(error);await loadModeration();}finally{auditingPostId.value=0;}}
...
function reviewReportedKnowledge(report:Report){const file=knowledgeFiles.value.find(item=>item.id===report.fileId);if(file)void reviewKnowledge(file);else ElMessage.warning('未找到被举报资源，可能已被删除');}
function reviewReportedUser(report:Report){const user=adminUsers.value.find(item=>item.id===report.targetUserId);if(user)void openUserGovernance(user);else ElMessage.warning('未找到被举报用户');}
...
async function toggleUserStatus(user:UserRecord){if(user.status==='ACTIVE'){if(user.id===currentUserId.value){ElMessage.warning('不能停用当前登录的管理员账号');return;}try{await ElMessageBox.confirm(`停用“${user.nickname}”后，该用户将无法登录。确定继续吗？`,'确认停用账号',{type:'warning',confirmButtonText:'停用账号'});}catch{return;}}await postData('/user/admin/status',{userId:user.id,status:user.status==='ACTIVE'?'DISABLED':'ACTIVE'});adminUsers.value=await getData('/user/admin/users');ElMessage.success('账号状态已更新');}
async function openUserGovernance(user:UserRecord){userGovernanceForm.value={userId:user.id,nickname:user.nickname,avatarUrl:user.avatarUrl||'',signature:user.signature||'',status:user.status,role:user.role,publishPolicy:user.publishPolicy||'STANDARD',messagingEnabled:user.messagingEnabled!==false,resetPassword:''};const [knowledge,posts,draftsResult,behaviorResult]=await Promise.all([getData<KnowledgeFile[]>(`/knowledge/mine?type=UPLOADED&userId=${user.id}`),getData<Post[]>(`/square/feed?authorUserId=${user.id}`),getData<Draft[]>(`/post/drafts?userId=${user.id}`),getData<BehaviorRecord[]>(`/user/behaviors?userId=${user.id}`)]);adminUserKnowledge.value=knowledge;adminUserPosts.value=posts;adminUserDrafts.value=draftsResult;adminUserBehaviors.value=behaviorResult;userGovernanceDialog.value=true;}
async function saveUserGovernance(){await postData('/user/admin/governance',userGovernanceForm.value);userGovernanceDialog.value=false;adminUsers.value=await getData('/user/admin/users');ElMessage.success('用户资料与权限已更新');}
function csvValue(value:unknown){let text=String(value??'');if(/^[=+\-@]/.test(text))text=`'${text}`;return `"${text.replace(/"/g,'""')}"`;}
function exportAdminUsers(){if(!filteredAdminUsers.value.length){ElMessage.warning('当前筛选条件下没有可导出的用户');return;}const rows=[['用户ID','用户名','昵称','角色','账号状态','发帖策略'],...filteredAdminUsers.value.map(user=>[user.id,user.username,user.nickname,roleLabel(user.role),user.status==='ACTIVE'?'正常':'已停用',publishPolicyLabel(user.publishPolicy)])];const blob=new Blob([`\ufeff${rows.map(row=>row.map(csvValue).join(',')).join('\r\n')}`],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download=`zhihui-users-${new Date().toISOString().slice(0,10)}.csv`;link.click();URL.revokeObjectURL(url);ElMessage.success(`已导出 ${filteredAdminUsers.value.length} 位用户`);}
async function loadTicketsAdmin(){[adminTickets.value,faqs.value,adminUsers.value]=await Promise.all([getData<Ticket[]>('/feedback/tickets'),getData<Faq[]>('/feedback/faqs'),getData<UserRecord[]>('/user/admin/users')]);}
...
async function loadAnalytics(){const [users,files,posts,ticketList,userOverview,knowledgeOverview,forumOverview,feedbackOverview]=await Promise.all([getData<UserRecord[]>('/user/admin/users'),getData<KnowledgeFile[]>('/knowledge/list?includeAll=true'),getData<Post[]>('/square/feed'),getData<Ticket[]>('/feedback/tickets'),getData<Record<string,unknown>>('/user/admin/overview'),getData<Record<string,unknown>>('/knowledge/admin/overview'),getData<Record<string,unknown>>('/post/admin/overview'),getData<Record<string,unknown>>('/feedback/admin/overview')]);adminUsers.value=users;knowledgeFiles.value=files;feedPosts.value=posts;adminTickets.value=ticketList;adminOverview.value={userAdmin:userOverview,knowledgeAdmin:knowledgeOverview,forumAdmin:forumOverview,feedbackAdmin:feedbackOverview};}
