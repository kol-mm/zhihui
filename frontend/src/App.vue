        ...
        <template v-else>
          ...
          <section v-else-if="activeView === 'moderation'" class="page-stack">
            ...
            <section class="surface"><el-tabs v-model="moderationTab">
              ...
              <el-tab-pane label="帖子" name="posts"><el-table :data="feedPosts"><el-table-column prop="id" label="ID" width="70" /><el-table-column prop="title" label="标题" min-width="220" /><el-table-column prop="userId" label="用户" width="90" /><el-table-column label="状态" width="110"><template #default="scope">{{ postStatusLabel(scope.row.status) }}</template></el-table-column><el-table-column label="操作" width="250"><template #default="scope"><el-button text type="primary" @click="reviewPost(scope.row)">查看内容</el-button><el-button v-if="scope.row.status === 'PENDING'" text type="success" @click="auditPost(scope.row, 'PUBLISHED')">发布</el-button><el-button v-if="scope.row.status === 'PENDING' || scope.row.status === 'PUBLISHED'" text type="danger" @click="auditPost(scope.row, 'HIDDEN')">{{ scope.row.status === 'PENDING' ? '驳回并隐藏' : '隐藏' }}</el-button><el-button v-else-if="scope.row.status === 'HIDDEN'" text type="success" @click="auditPost(scope.row, 'PUBLISHED')">恢复</el-button></template></el-table-column></el-table></el-tab-pane>
            </el-tabs></section>
          </section>
...
onMounted(async()=>{syncUserForms();window.addEventListener('popstate',handlePopState);await loadPublicConfig();await loadCaptcha();if(authenticated.value)await restoreSession();});
