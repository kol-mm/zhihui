...
const aiConfig = ref({ data_source_scope:'all-approved', match_limit:5, compliance_rule:'answer-with-references', provider:'local', model:'local-rag', base_url:'', temperature:0.2, max_upload_mb:25, notifications_enabled:true, community_enabled:true });
...
function handleKnowledgeFile(file:UploadFile){selectedKnowledgeFile.value=file.raw; if(file.raw&&!knowledgeForm.value.title)knowledgeForm.value.title=file.name.replace(/\.[^.]+$/,'');}
