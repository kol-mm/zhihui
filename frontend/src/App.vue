...
type UserRecord = { id:number; username:string; nickname:string; avatarUrl?:string; signature?:string; status:string; role:string; publishPolicy?:string; messagingEnabled?:boolean; email?:string|null; emailVerified?:boolean };
...
async function saveUserGovernance(){const {email:_email,emailVerified:_verified,...changes}=userGovernanceForm.value;await postData('/user/admin/governance',changes);userGovernanceDialog.value=false;await loadAdminUsers(true);ElMessage.success('用户资料与权限已更新');}
