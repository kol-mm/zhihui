    ...
    <section class="workspace-shell">
      ...
      <main class="content-area">
        ...
        <template v-else>
          ...
          <section v-else class="page-stack">
            <div class="page-toolbar"><div><h3>平台与 AI 配置</h3><p>统一管理平台开放范围、审核规则、容量限制和知识检索</p></div><el-button type="primary" @click="saveAiConfig">保存全部配置</el-button></div>
...
function selectView(key:string){ if(key==='governance'){governanceDialog.value=true;mobileMenuOpen.value=false;loadGovernance();return;} if(key==='notifications'){mobileMenuOpen.value=false;openNotifications();return;} returnToRoot();if(key==='forum')feedMode.value='all';if(key==='square')feedMode.value='following';activeView.value=key; mobileMenuOpen.value=false; mobileAiHistoryOpen.value=false; window.scrollTo({top:0,behavior:'smooth'}); refreshCurrentView(); }
...
onMounted(async()=>{syncUserForms();window.addEventListener('popstate',handlePopState);window.addEventListener('keydown',handleGlobalKeydown);await loadPublicConfig();if(authenticated.value)await restoreSession();else await loadCaptcha();});
onBeforeUnmount(()=>{window.removeEventListener('popstate',handlePopState);window.removeEventListener('keydown',handleGlobalKeydown);document.body.classList.remove('mobile-menu-active');if(captchaCooldownTimer)clearInterval(captchaCooldownTimer);if(captchaExpiryTimer)clearTimeout(captchaExpiryTimer);if(pdfPreviewUrl.value)URL.revokeObjectURL(pdfPreviewUrl.value);if(detailPdfPreviewUrl.value)URL.revokeObjectURL(detailPdfPreviewUrl.value);});
</script>
