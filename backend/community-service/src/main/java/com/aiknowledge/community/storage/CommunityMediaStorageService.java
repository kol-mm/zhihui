package com.aiknowledge.community.storage;
...
public class CommunityMediaStorageService {
    ...
    public CommunityMediaStorageService(
            @Value("${community.media.storage-mode:local}") String storageMode,
            @Value("${community.media.local-root:../data/community-media}") String localRoot,
            @Value("${community.media.minio.endpoint:http://127.0.0.1:9000}") String endpoint,
            @Value("${community.media.minio.bucket:ai-community}") String bucket,
            @Value("${community.media.minio.access-key:aiknowledge}") String accessKey,
            @Value("${community.media.minio.secret-key:ai-knowledge-local-change-me}") String secretKey
    ) {
    ...
    public String save(String filename, byte[] bytes) {
        ...
        String objectName = LocalDate.now().format(DATE_PATH) + "/" + UUID.randomUUID() + "." + type.extension();
        try {
            ...
            return "/post/media/" + Base64.getUrlEncoder().withoutPadding()
                    .encodeToString(objectName.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        } catch (Exception error) {
            throw new IllegalStateException("failed to store community image", error);
        }
    }
    ...
    public StoredMedia read(String token) {
        try {
            String objectName = new String(Base64.getUrlDecoder().decode(token), java.nio.charset.StandardCharsets.UTF_8);
            if (!objectName.matches("\\d{4}/\\d{2}/\\d{2}/[a-f0-9-]+\\.(jpg|png|gif|webp)")) {
            ...
            throw new IllegalStateException("failed to read community image", error);
        }
    }
