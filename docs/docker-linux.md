...
管理员在后台做的账号处理、内容审核、删除和平台设置修改都会记入“操作记录”页面（谁、何时、从哪个地址、改了什么）。记录默认保留 365 天，可在 `.env` 中用 `AUDIT_RETENTION_DAYS` 调整（0 表示永久保留）。记录由用户服务保存在 `user_db.admin_audit_log`，随数据库一起备份；其他服务暂时连不上用户服务时，会把未能送达的记录完整写入各自的容器日志（`docker compose logs <服务名> | grep "Admin audit"`）。
