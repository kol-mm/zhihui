...
- 检查粉丝、拉黑、用户举报和互动足迹均可保存并重新加载。
- 发布包含多张图片的帖子，检查关注动态、作者筛选和帖子点赞。
- 上传包含正文图片的 DOCX，检查用户阅读和管理员审核均按文档顺序显示文字与图片；删除资源后确认图片不可再访问。
- 删除单条消息、清空指定会话及全部会话，并查看 AI 问答历史。
...
- 结案知识举报、删除违规评论/废弃草稿，并新增、编辑或删除 FAQ。
- 配置 AI 数据源范围、匹配数量和合规规则，查看问答记录与知识切片。
