...
- 输入本地文件内容提交知识，确认返回 `local-file://` 文件地址。
- 使用“全文搜索”检索知识正文。
- 发布帖子，刷新广场后能看到帖子。
- 对帖子发表评论，查看详情和评论列表。
- 保存草稿并查看“我的草稿”。
...
- 检查 `/ai/vector/status`，确认向量维度和已索引切片数量。
...
- 容器化部署暂不做。
- 文件存储、消息事件、全文检索、向量检索、JWT/RBAC 已完成本地可用实现。
- MinIO、RabbitMQ、Elasticsearch、Milvus/ChromaDB 保留可配置接入点，默认本地模式不依赖这些外部进程。
